...
const MelaninTest = ({result,setResult}) => {
    ...
    useEffect(() => {
        ...
        axios.get('/questions?category=pigmentation')
            .then(response => {
    ...
    <div className="title">
      ...
      <div className="progress-bar">
        ...
        <div className="balance-test-container"> 
            <h2 className="balance-test-title">멜라닌 색소 테스트</h2>
            {questions.map((question, index) => (
                <div key={question.id} className="question-container">
                    <Question question={question} handleAnswer={handleAnswer} questionNumber={index + 1}/>
                </div>
            ))}
            <button className="submit-button" onClick={pigmentationResult}>제출</button> 
        </div>
        </div>
    );
};

export default MelaninTest;
