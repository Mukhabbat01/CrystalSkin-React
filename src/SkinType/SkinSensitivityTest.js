...
const SkinSensitivityTest = ({result, setResult}) => {
    const [questions, setQuestions] = useState([]);
    const [answers, setAnswers] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        console.log(`${result} 1번째 결과값`)
        axios.get('/questions?category=sensitivity')
            .then(response => {
    ...
    <div className="title">
      ...
      <div className="progress-bar">
        ...
        <div className="balance-test-container"> 
            <h2 className="balance-test-title">피부 민감도 테스트</h2>
            {questions.map((question, index) => (
                <div key={question.id} className="question-container">
                    <Question question={question} handleAnswer={handleAnswer}  questionNumber={index + 1} />
                </div>
            ))}
            <button className="submit-button" onClick={sensitivityResult}>제출</button> 
        </div>
        </div>
    );
};

export default SkinSensitivityTest;
